...
export function designIntent(doc, build) {
  ...
  return {
    ...
    document: {
      ...
      unitDefinition: tfmt('1 mm = {p1} {units}. Every length in this file is in millimetres, which is what the engine stores; the unit above is only how they are displayed.', { p1: UNITS[doc.meta.units]?.perMm ?? 1, units: doc.meta.units }),
...
function readme(doc, build, report, estimate, cross, opts) {
  ...
  L.push(tfmt('Release package generated by {APP_NAME} {APP_VERSION} on {p1}.', { APP_NAME, APP_VERSION, p1: new Date().toISOString().slice(0, 10) }), '');
  ...
  L.push(tfmt('| `{p1}` | The editable source document. |', { p1: safeName(doc.meta.name, FILE_EXT) }));
  ...
  if (!report.issues.length) {
    L.push(tfmt('All {checked} checks passed.', { checked: report.checked }), '');
  } else {
    ...
    if (report.issues.length > 25) L.push(tfmt('- …and {p1} more in `checks.csv`.', { p1: report.issues.length - 25 }));
    L.push('');
  }

  L.push('## Manufacturing basis', '');
  L.push(`- Process assumed: **${p.label}**`);
  L.push(tfmt('- Minimum wall: {minWall}{u}, minimum feature: {minFeature}{p1}, tolerance ±{tolerance}{p2}', { minWall: lim.minWall, u, minFeature: lim.minFeature, p1: u, tolerance: lim.tolerance, p2: u }));
  ...
  if (estimate.rows.length) {
    ...
    L.push(tfmt('**{p1}{p2} per unit at a batch of {batch}**, total {p3}.', { p1: estimate.each.toFixed(2), p2: s.currency ? ' ' + s.currency : '', batch: estimate.batch, p3: estimate.total.toFixed(2) }), '');
    ...
    if (cross?.changes.length) {
      ...
      for (const c of cross.changes) {
        L.push(tfmt('- Between {qty} and {p1} off, **{label}** overtakes {p2}.', { qty: c.from.qty, p1: c.to.qty, label: c.to.label, p2: c.from.label }));
      }
      L.push('');
    }
  ...
  if (opts.mesh) {
    ...
    L.push('## Mesh', '');
    L.push(tfmt('- Quality: **{label}**{tol}', { label: q.label, tol: q.tol ? tfmt(', chord tolerance {mm} mm', { mm: q.tol }) : '' }));
    if (q.tol) L.push(tfmt('- No point on `model.stl` is more than {tol} mm from the surface it represents.', { tol: q.tol }));
    L.push(tfmt('- {p1} triangles after welding, from {p2}{p3}.', { p1: opts.mesh.after.toLocaleString(), p2: opts.mesh.before.toLocaleString(), p3: opts.mesh.dropped ? ` (${opts.mesh.dropped} degenerate removed)` : '' }));
    L.push('');
  ...
  L.push('');
  L.push(t('Mass is computed from the mesh and the assigned material density, so it is exact for the geometry shown and only as good as the material choice.'), '');
